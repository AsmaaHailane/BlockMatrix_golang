...
func (bm *BlockMatrix) updateRowHashes(From int, To int) {
	...
	if TraceEnabled {
		log.Printf("updateRowHashes(%d, %d) called\n", From, To)
	}
...
func (bm *BlockMatrix) updateColumnHashes(From int, To int) {
	...
	if TraceEnabled {
		log.Printf("updateColumnHashes(%d, %d) called\n", From, To)
	}
...
func (bm *BlockMatrix) blockIndex(BlockNumber int) (i, j int) {
	...
	if TraceEnabled {
		log.Printf("blockIndex(%d) called\n", BlockNumber)
	}
	...
	if TraceEnabled {
		log.Printf("blockIndex() returning %d, %d\n", i, j)
	}
...
func (bm *BlockMatrix) GetBlockData(BlockNumber int) bytes.Buffer {
	...
	if TraceEnabled {
		log.Printf("GetBlockData(%d) called\n", BlockNumber)
	}
	...
	if TraceEnabled {
		log.Printf("GetBlockData() returning %v\n", &bm.BlockData[i][j])
	}
...
func (bm *BlockMatrix) GetBlockHash(BlockNumber int) string {
	...
	if TraceEnabled {
		log.Printf("GetBlockHash(%d) called\n", BlockNumber)
	}
	...
	if TraceEnabled {
		log.Printf("GetBlockHash() returning %v\n", &bm.BlockHashes[i][j])
	}
...
func (bm *BlockMatrix) DeleteBlock(BlockNumber int) bool {
	...
	if TraceEnabled {
		log.Printf("DeleteBlock(%d) called\n", BlockNumber)
	}
	...
	if i < 0 || j < 0 || i > bm.Dimension || j > bm.Dimension {
		...
		if TraceEnabled {
			log.Printf("DeleteBlock() invalid BlockNumber %d\n", BlockNumber)
		}
...
func (bm *BlockMatrix) deleteBlockAt(RowNumber int, ColNumber int) bool {
	...
	if RowNumber < 0 || ColNumber < 0 || RowNumber > bm.Dimension || ColNumber > bm.Dimension {
		...
		if TraceEnabled {
			log.Printf("deleteBlockAt() invalid RowNumber %d or ColNumber %d\n", RowNumber, ColNumber)
		}
	...
	if TraceEnabled {
		log.Printf("deleteBlockAt(%d, %d) successful\n", RowNumber, ColNumber)
	}
...
func (bm *BlockMatrix) Dump(MaxChars int) {
	...
	for i := 0; i < bm.Dimension; i++ {
		...
		if bm.RowHashes[i] != "" {
			fmt.Printf("RowHashes[%d]: %s\n", i, bm.RowHashes[i][0:MaxChars])
		}
	}

	for j := 0; j < bm.Dimension; j++ {

		if bm.ColumnHashes[j] != "" {
			fmt.Printf("ColumnHashes[%d]: %s\n", j, bm.ColumnHashes[j][0:MaxChars])
		}
	}
...
func (bm *BlockMatrix) writeLockBlockAt(RowNumber int, ColNumber int) bool {
	...
	if RowNumber < 0 || ColNumber < 0 || RowNumber > bm.Dimension || ColNumber > bm.Dimension {
		...
		if TraceEnabled {
			log.Printf("writeLockBlockAt() invalid RowNumber %d or ColNumber %d\n", RowNumber, ColNumber)
		}
...
func (bm *BlockMatrix) writeUnlockBlockAt(RowNumber int, ColNumber int) bool {
	...
	if RowNumber < 0 || ColNumber < 0 || RowNumber > bm.Dimension || ColNumber > bm.Dimension {
		...
		if TraceEnabled {
			log.Printf("writeUnlockBlockAt() invalid RowNumber %d or ColNumber %d\n", RowNumber, ColNumber)
		}
...
func (bm *BlockMatrix) readLockBlockAt(RowNumber int, ColNumber int) bool {
	...
	if RowNumber < 0 || ColNumber < 0 || RowNumber > bm.Dimension || ColNumber > bm.Dimension {
		...
		if TraceEnabled {
			log.Printf("readLockBlockAt() invalid RowNumber %d or ColNumber %d\n", RowNumber, ColNumber)
		}
...
func (bm *BlockMatrix) readUnlockBlockAt(RowNumber int, ColNumber int) bool {
	...
	if RowNumber < 0 || ColNumber < 0 || RowNumber > bm.Dimension || ColNumber > bm.Dimension {
		...
		if TraceEnabled {
			log.Printf("readUnlockBlockAt() invalid RowNumber %d or ColNumber %d\n", RowNumber, ColNumber)
		}
